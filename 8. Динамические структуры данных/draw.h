...
#define SCALE 21									//coordinate scaling factor
#define WINDOW 500									//Size of Window
...
void drawText(char data, int x0, int y0, bool searched) {

	GLvoid* font_style = GLUT_BITMAP_TIMES_ROMAN_24;
	y0 = WINDOW / 2 - y0 - RADIUS / 2;
	x0 = x0 - RADIUS / 2;                                             //bring str to center of circle
	glRasterPos2f(x0, y0);
	glutBitmapCharacter(font_style, data);
...
void draw() {
	...
	while (!Q.empty()) {							//BFS of tree
		...
		drawText(root->data, x0, y0, root->searched);			//draw node value
